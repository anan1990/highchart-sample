...
Liquidity.prototype.init = function() {
	...
	$.get( mkwSocket + "/mkwsocket/liquidity?index=PRE-" + index, function( response ) {
		...
		if (!self.arrDataLiY || self.arrDataLiY.length === 0) {
			...
			$.get( mkwSocket + "/mkwsocket/liquidity?index=" + index, function( response ) {
...
Liquidity.prototype.createChartData = function() {
	...
	if (this.arrDataLi.length > 0) {
		for ( var i in this.arrDataLi) {
			ranges.push([parseFloat(this.arrDataLi[i].time) , 0, parseFloat(this.arrDataLi[i].value.toFixed(2))]);
		}
	}
	...
	if (this.arrDataLiY.length > 0) {
		for ( var i in this.arrDataLiY) {
			...
			ranges1.push([parseFloat(this.arrDataLiY[i].time), 0, parseFloat(this.arrDataLiY[i].value.toFixed(2))]);
		}
	}
